...
# Optional: Add a marker to skip VCR for specific tests
def pytest_configure(config):
    config.addinivalue_line(
        "markers", "no_vcr: mark test to not use VCR recording/playback"
    )


# Optional: Skip VCR for tests marked with @pytest.mark.no_vcr
@pytest.hookimpl(tryfirst=True)
def pytest_runtest_setup(item):
    if any(marker.name == "no_vcr" for marker in item.iter_markers()):
        vcr.disable_vcr()
