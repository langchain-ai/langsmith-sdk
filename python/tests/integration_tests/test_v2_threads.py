...
@pytest.fixture
async def project_with_thread(langchain_client: Client):
    ...
    async def _runs_indexed() -> Optional[str]:
        """Return the project id once at least one of its runs is queryable."""
        project_id = str(langchain_client.read_project(project_name=project_name).id)
        async for _ in langchain_client.runs.query(
    ...
    project_id = await _wait_for_async(_runs_indexed, max_sleep_time=90, sleep_time=2)
    try:
        yield project_id, thread_id, min_start_time, max_start_time
    finally:
        if langchain_client.has_project(project_name=project_name):
            langchain_client.delete_project(project_name=project_name)
