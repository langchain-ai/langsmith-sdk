...
def test_otel_trace_export(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("LANGCHAIN_OTEL_ENABLED", "true")

    client = Client(api_key="test-key")

    assert client.otel_exporter is not None, "OTEL exporter should be enabled."
    ...
    # Wait until the run is ingested by LangSmith.
    wait_for(lambda: client.read_run(run_id).end_time is not None)

    # Retrieve the run from LangSmith.
    retrieved_run = client.read_run(run_id)
    assert retrieved_run.id == run_id
    assert retrieved_run.start_time == start_time
    assert retrieved_run.end_time == end_time
    assert retrieved_run.name == "OTEL Export Test"
    assert retrieved_run.run_type == "llm"
    assert retrieved_run.inputs == {"prompt": "Hello, OTEL!"}
    assert retrieved_run.outputs == {"answer": "Hello, User!"}
    assert retrieved_run.extra == {"metadata": {"foo": "bar"}}
    assert retrieved_run.tags == ["otel", "test"]
