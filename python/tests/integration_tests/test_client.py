...
def test_update_dataset_schema(langchain_client: Client) -> None:
    ...
    try:
        ...
        # Verify old examples are still valid (server doesn't validate
        # existing examples against new schema)
...
def test_update_dataset_by_name(langchain_client: Client) -> None:
    ...
    try:
        # Create dataset with initial values
        dataset = langchain_client.create_dataset(
            dataset_name=dataset_name, description="Initial description"
        )

        # Update the dataset using name instead of ID
        updated_dataset = langchain_client.update_dataset(
            dataset_name=dataset_name, name=new_name, description="Updated description"
        )
...
def test_update_dataset_errors(langchain_client: Client) -> None:
    ...
    with pytest.raises(LangSmithNotFoundError):
        langchain_client.update_dataset(dataset_id=non_existent_id, name="New name")
    ...
    with pytest.raises(LangSmithNotFoundError):
        langchain_client.update_dataset(
            dataset_name=non_existent_name, description="Updated description"
        )
    ...
    try:
        # Create two datasets
        langchain_client.create_dataset(dataset_name=dataset_name1)
        ...
        with pytest.raises(LangSmithConflictError):
            langchain_client.update_dataset(dataset_id=dataset2.id, name=dataset_name1)
    finally:
        # Clean up
        try:
