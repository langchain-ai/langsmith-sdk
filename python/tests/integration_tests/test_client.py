...
def _setup_experiment(
    langchain_client: Client, dataset_name: str, num_examples: int = 3
) -> tuple:
    """Create a dataset with examples, run an experiment, return (dataset, experiment_id)."""
    dataset = _create_dataset(langchain_client, dataset_name)
    langchain_client.create_examples(
    ...
    results = langchain_client.evaluate(lambda i: {"a": "x"}, data=dataset_name)
    project = langchain_client.read_project(project_name=results.experiment_name)
    return dataset, str(project.id)


def test_v2_experiment_runs_create(langchain_client: Client) -> None:
    """Test the v2 experiment-runs endpoint: response shape and basic fetch."""
    if not hasattr(langchain_client._get_langsmith_api().datasets, "experiment_runs"):
        pytest.skip("requires regenerated SDK with experiment_runs resource")

    dataset_name = "__test_v2_exp_runs_" + uuid7().hex
    dataset, experiment_id = _setup_experiment(langchain_client, dataset_name)

    def _runs_ready() -> bool:
        page = langchain_client._get_langsmith_api().datasets.experiment_runs.create(
            str(dataset.id), experiment_ids=[experiment_id], page_size=10
        )
        return len(page.items) > 0

    wait_for(_runs_ready, max_sleep_time=30)
...
def test_v2_experiment_runs_cursor_pagination(langchain_client: Client) -> None:
    """Test v2 experiment-runs cursor pagination: page_size=1 should yield 3 sequential pages."""
    if not hasattr(langchain_client._get_langsmith_api().datasets, "experiment_runs"):
        pytest.skip("requires regenerated SDK with experiment_runs resource")
...
def test_v1_runs_create_backward_compat(langchain_client: Client) -> None:
    """Regression: v1 datasets.runs.create still returns a bare list (not {items:...})."""
    ...
    def _runs_ready() -> bool:
        resp = langchain_client._get_langsmith_api().datasets.runs.create(
