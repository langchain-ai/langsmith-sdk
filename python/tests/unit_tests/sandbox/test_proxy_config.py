...
def test_proxy_config_ignores_deprecated_no_proxy() -> None:
    with pytest.warns(DeprecationWarning, match="no_proxy"):
        config = proxy_config(rules=[], no_proxy=["metadata.google.internal"])
    assert "no_proxy" not in config
