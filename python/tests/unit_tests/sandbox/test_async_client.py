...
class TestAsyncConnectOperation:
    """Tests for async connect operation."""
    ...
    async def test_connect_to_existing_sandbox(
        ...
        sandbox = await client.connect("my-sandbox")

        assert sandbox.name == "my-sandbox"
        assert sandbox.template_name == "python-sandbox"
        assert (
            sandbox.dataplane_url == "https://sandbox-router.example.com/tenant/sb-123"
        )

    async def test_connect_not_found(
        self, client: AsyncSandboxClient, httpx_mock: HTTPXMock
    ):
        """Test connecting to a non-existent sandbox."""
        httpx_mock.add_response(
            method="GET",
            url="http://test-server:8080/boxes/nonexistent",
            json={"detail": "Sandbox 'nonexistent' not found"},
    ...
    async def test_connect_auto_delete_false_by_default(
        self, client: AsyncSandboxClient, httpx_mock: HTTPXMock
    ):
        """Test that auto_delete defaults to False."""
        httpx_mock.add_response(
            method="GET",
            url="http://test-server:8080/boxes/my-sandbox",
            json={
        ...
        # Use async context manager and verify no delete is called
        async with sandbox:
            pass

        # Verify no DELETE request was made
        requests = httpx_mock.get_requests()
        assert len(requests) == 1  # Only the GET request
        assert requests[0].method == "GET"

    async def test_connect_with_auto_delete_true(
        self, client: AsyncSandboxClient, httpx_mock: HTTPXMock
    ):
        """Test connecting with auto_delete=True deletes on context exit."""
        httpx_mock.add_response(
            method="GET",
            url="http://test-server:8080/boxes/my-sandbox",
            json={
        ...
        sandbox = await client.connect("my-sandbox", auto_delete=True)
