...
class TestConnectOperation:
    ...
    def test_connect_to_existing_sandbox(
        self, client: SandboxClient, httpx_mock: HTTPXMock
    ):
        ...
        sandbox = client.connect("my-sandbox")

        assert sandbox.name == "my-sandbox"
        assert sandbox.template_name == "python-sandbox"
        assert sandbox.dataplane_url == "https://sandbox-router.example.com/sb-123"

    def test_connect_not_found(self, client: SandboxClient, httpx_mock: HTTPXMock):
    ...
    def test_connect_auto_delete_false_by_default(
        self, client: SandboxClient, httpx_mock: HTTPXMock
    ):
        ...
        # Use context manager and verify no delete is called
        with sandbox:
            pass
    ...
    def test_connect_with_auto_delete_true(
        self, client: SandboxClient, httpx_mock: HTTPXMock
    ):
        ...
        sandbox = client.connect("my-sandbox", auto_delete=True)
