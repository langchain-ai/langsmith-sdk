...
def test_validate_api_key_if_hosted(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    client = LangChainPlusClient(api_url="http://127.0.0.1:1984")
    assert client.api_url == "http://127.0.0.1:1984"
    assert client.api_key is None


def test_headers(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("LANGCHAIN_API_KEY", raising=False)
    client = LangChainPlusClient(api_url="http://127.0.0.1:1984", api_key="123")
    assert client._headers == {"x-api-key": "123"}

    client_no_key = LangChainPlusClient(api_url="http://127.0.0.1:1984")
