...
def test_set_metadata_skips_none_values(mock_exporter_cls):
    """Test that None values in metadata are not set on spans."""
    mock_inner_processor = MagicMock()
    processor = OtelSpanProcessor(
        api_key="test", project="test", SpanProcessor=lambda _: mock_inner_processor
    )

    processor.set_metadata({"thread_id": "t-123", "ignored": None})

    span = MagicMock()
    processor.on_start(span, parent_context=None)

    span.set_attribute.assert_called_once_with("langsmith.metadata.thread_id", "t-123")
...
def test_no_metadata_no_attributes(mock_exporter_cls):
    """Test that on_start sets no attributes when metadata is empty."""
    mock_inner_processor = MagicMock()
    processor = OtelSpanProcessor(
        api_key="test", project="test", SpanProcessor=lambda _: mock_inner_processor
    )
