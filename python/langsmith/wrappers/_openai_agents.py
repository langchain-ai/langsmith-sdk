...
def _extract_generation_span_data(span_data: tracing.GenerationSpanData) -> Dict[str, Any]:
    data = {
        "inputs": span_data.input,
        "outputs": span_data.output,
        "metadata": {
...
def _extract_response_span_data(span_data: tracing.ResponseSpanData) -> Dict[str, Any]:
    ...
    if span_data.response is not None:
        ...
        data["metadata"].update(span_data.response.model_dump(exclude={"input", "output", "metadata", "usage"}))
...
def _extract_guardrail_span_data(span_data: tracing.GuardrailSpanData) -> Dict[str, Any]:
    return {
        "metadata": {
    ...
    if isinstance(span.span_data, tracing.GenerationSpanData):
        ...
        if usage:
            usage_metadata = {
                ...
                "input_tokens": usage.get("prompt_tokens") if isinstance(usage, dict) else getattr(usage, "prompt_tokens", None),
                "output_tokens": usage.get("completion_tokens") if isinstance(usage, dict) else getattr(usage, "completion_tokens", None),
            }
...
class LangsmithTracingProcessor(tracing.TracingProcessor):
    """
    LangsmithTracingProcessor is a TracingProcessor for the OpenAI Agents SDK that logs traces and spans
    to Langsmith.
