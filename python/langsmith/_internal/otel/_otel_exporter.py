...
class OTELExporter:
    ...
    def __init__(self, tracer_provider=None, max_spans=10000, span_ttl_seconds=3600):
        ...
        Args:
            ...
            max_spans: Maximum number of spans to keep in memory (default: 10000)
            span_ttl_seconds: Time-to-live for incomplete spans in seconds (default: 3600 = 1 hour)
        """
    ...
    def export_batch(
        ...
        """
        # Cleanup expired and excess spans before processing new operations
    ...
    def _cleanup_stale_spans(self) -> None:
        ...
        if len(self._spans) - len(spans_to_remove) > self._max_spans:
            ...
            remaining_spans = [
                (span_id, ts) for span_id, ts in self._span_timestamps.items() 
