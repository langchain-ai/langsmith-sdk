"""Thread-local storage utilities for Google ADK tracing.

This module provides thread-local storage for the parent run tree,
which is used by callbacks to maintain trace context when async context
...
# Thread-local store for passing the parent run tree into callbacks.
# Google ADK's async event loop may break tracing contextvars.
# The parent run tree is threaded via thread-local as a fallback
# when context propagation isn't available.
