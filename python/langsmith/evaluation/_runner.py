...
# Row-level evaluator
COMPARATIVE_EVALUATOR_T = Callable[[List[schemas.Run], Optional[schemas.Example]], EvaluationResult]
...
def evaluate_pairwise(
    ...
    Args:
        experiments (Tuple[Union[str, uuid.UUID], Union[str, uuid.UUID]]): The identifier of the experiment to evaluate.
        data (DATA_T): The data to use for evaluation.
        evaluators (Optional[Sequence[EVALUATOR_T]]): Optional sequence of evaluators to use for individual run evaluation.=
        metadata (Optional[dict]): Optional metadata to include in the evaluation results.
        max_concurrency (Optional[int]): Optional maximum number of concurrent evaluations.
    ...
    runs = [sorted(single_exp_runs, key=lambda r: str(r.reference_example_id)) for single_exp_runs in runs]
    data = sorted(data, key=lambda d: str(d.id))

    runs_dict = collections.defaultdict(list)
    for runs_list in runs:
        for run in runs_list:
            runs_dict[run.reference_example_id].append(run)
...
def _evaluate_multi(
    target: List[Iterable[schemas.Run]],
    /,
    data: DATA_T,
    evaluators: Optional[Sequence[EVALUATOR_T]] = None,
    metadata: Optional[dict] = None,
    ...
    # Initialize the experiment manager.
    client = client or langsmith.Client()
    runs = cast(Iterable[schemas.Run], target[0])
...
class _ComparativeExperimentManagerMixin:
    def __init__(
        self,
        /,
        experiment: Optional[Union[schemas.TracerSession, str]],
...
class _ComparativeExperimentManager(_ExperimentManagerMixin):
    """Manage the execution of experiments.
