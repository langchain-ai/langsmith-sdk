...
class AsyncSandbox:
    ...
    async def list_commands(self, *, timeout: int = 60) -> list[CommandInfo]:
        ...
        try:
            response = await self._client._http.get(url, timeout=timeout)
            response.raise_for_status()
            data = response.json()
    ...
    async def get_command(
        self, command_id: str, *, timeout: int = 60
    ) -> CommandInfo:
