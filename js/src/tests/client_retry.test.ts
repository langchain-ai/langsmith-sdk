...
describe("Client timeout and retry behavior", () => {
  ...
  beforeAll((done) => {
    testServer = createServer((req, res) => {
      ...
      if (testMode === "timeout") {
        ...
        if (requestCount <= 2) {
          // First two requests: return 408 (should retry)
  ...
  it("should retry on 408 timeout status and eventually succeed", async () => {
  ...
  it("should not retry on 400 Bad Request", async () => {
    ...
    // Override caller with retries enabled
    (client as any).caller = new AsyncCaller({
      maxRetries: 3,
      maxConcurrency: 1,
    });
